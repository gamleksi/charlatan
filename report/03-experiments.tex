...
We use the triplet loss presented in the previous section. At each epoch, we create a dataset of 1000 triplets. We then run stochastic gradient descent with momentum against the triplet loss over this dataset 5 times after which a new triplet set is sampled and the process is repeated.
...
The reward calculated using the huber loss presented in section \ref{sec:methods} using the embedding of the image robot and the embedding of the video frame.

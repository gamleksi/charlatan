...
The output of the policy network is a 7-dimensional vector of which values determine the mean for sampling actions from a \textit{Gaussian} distribution. The action values determine forces of the motors of the seven joints. The action values are between -200 and 200. \textit{Hyperbolic tangent function} is used for the activation function in the fully connected policy network. We experimented different layer structures for the learning problem. As the complexity of the action space is high, the best result was achieved with 3 hidden layers with width 128. The same input value is used for the value network. The network returns recommendation of the state in scalar. The value network shares the same features as the policy network but it has 2 hidden layers.
...
\subsection{Goal pose expperiment}
